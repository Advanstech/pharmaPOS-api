...
// Swagger UI documents REST health probes + this full GraphQL reference.
...
async function bootstrap() {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    // Development: Disable CSP for GraphQL Playground
  ...
  // ── Swagger ───────────────────────────────────────────────────────────────
  // REST endpoints (health probes) + full GraphQL operations reference.
  // Available in all environments — restrict to internal network in production.
    ...
    .addBearerAuth(
      {
        ...
        in: 'header',
        description:
          'JWT access token from the `login` GraphQL mutation.\n' +
